...
const Mining = () => {
  ...
  // Calculate available balance (USDT balance + any approved loan amounts)
  const usdtBalance = getBalance('USDT');
  const totalLoanAmount = activeLoans.reduce((sum, loan) => sum + Number(loan.amount), 0);
  const availableForMining = usdtBalance + totalLoanAmount;
          ...
          {!isEligible && (
            <div className="bg-orange-500/10 border border-orange-500/30 rounded-2xl p-6 mb-8">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-orange-600 dark:text-orange-400">Mining Access Restricted</p>
                  <p className="text-sm text-orange-600/80 dark:text-orange-400/80 mt-1">
                    Mining is only available for users with total deposits exceeding 5,000 USDT. 
            ...
            <>
                  ...
                  <CardContent>
                    ...
                    {totalLoanAmount > 0 && (
                      <p className="text-xs text-muted-foreground mt-1">
                        Includes {totalLoanAmount.toLocaleString()} USDT from loans
                      </p>
                    )}
                  </CardContent>
                </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {MINING_TIERS.map((tier) => (
                    <Card 
                      ...
                      onClick={() => handleTierSelect(tier.tier as 1 | 2 | 3)}
                    >
                      <CardHeader>
              ...
              <Card className="mb-8">
                ...
                <CardContent className="space-y-2 text-sm text-muted-foreground">
                  ...
                  <p>• Standard withdrawal restrictions (active loans, credit score) still apply after maturity.</p>
                </CardContent>
              </Card>
              ...
              {/* History */}
              {investments.filter(inv => ['completed', 'settled', 'rejected'].includes(inv.status)).length > 0 && (
                <div>
